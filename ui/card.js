...
function persist(){const open=[...root.querySelectorAll('details.panel[open]')].map(x=>x.dataset.key);window.openai?.setWidgetState?.({privateContent:{open},modelContent:`《万象历》已展开栏目：${open.join('、')||'无'}`})}
function render(v){if(!v||v.schema!=='wanxiangli/2')return;root.replaceChildren();const f=v.four_signs,l=v.three_lenses,r=v.reality,d=v.date;
 add(root,'div','eyebrow','万象历 · 每日观象');add(root,'div','date',`${d.gregorian} · ${d.lunar} · ${d.ganzhi.day}日`);
 add(root,'h1','rating',`今日 · ${v.rating.rating}`);add(root,'div','focus',v.focus.join(' · '));
 const actions=add(root,'div','actions');for(const [label,list] of [['宜',r.effective_dos],['忌',r.effective_donts]]){const box=add(actions,'div','action');add(box,'strong','',label);add(box,'span','',list.slice(0,2).join(' · '))}
 ...
 const q=f.daoism;panel('daoism','道教',`${q.title} · ${q.grade}`,box=>{p(box,`原签诗：${q.verse}`);p(box,`今日解释：${q.reading}`)}, {lines:[q.selection,`目前仅核对入库 ${q.corpus_size} 条签诗；原书共 ${q.historical_corpus_size} 条。长篇断语未作预测。`],links:[['《玄真灵应宝签》签页',q.source_url]]});
 ...
 const c=l.christianity;panel('christianity','基督宗教',`今日箴言 · ${c.reference}`,box=>{p(box,`原创中文转述：${c.paraphrase}`);p(box,`默想：${c.reflection}`)}, {lines:[c.method,'使用独立经节池，不复制摩拉维亚教会每日序列；不预测未来，也不代表任何教会。'],links:[['Moravian Daily Watchwords','https://www.moravian.org.uk/daily-watchwords/what-are-the-daily-watchwords'],['World English Bible','https://ebible.org/eng-web/']]});
...
if(window.openai?.toolOutput?.schema==='wanxiangli/2')render(window.openai.toolOutput);
