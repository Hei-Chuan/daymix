...
registerAppTool(server,'get_daily_card',{
 title:'查看今日运势 · 万象历',description:'Generate one stable Wanxiangli v2 daily card with four signs, three reflective lenses, astronomical positions, sources and a local expandable UI.',
 ...
 const plain=[`万象历 ${ledger.date.gregorian} · ${ledger.rating.rating}`,
  `宜 ${r.effective_dos.join(' · ')}`,`忌 ${r.effective_donts.join(' · ')}`,
  `四象：儒家·易 ${y.primary.name}→${y.changed.name}（动爻 ${y.moving_lines.join('、')||'无'}）`,
  `塔罗 ${t.name} · ${t.orientation}；道教 ${ledger.four_signs.daoism.title} · ${ledger.four_signs.daoism.grade}`,
  `星象 月亮${ledger.four_signs.astrology.positions.moon.zodiac_sign}，主要相位 ${ledger.four_signs.astrology.aspects.length} 项`,
  `三镜：佛教察「${l.buddhism.focus}」；基督宗教 ${l.christianity.reference}；斯多葛关注可控行动`,
...
registerAppResource(server,'wanxiangli-card',uri,{mimeType:RESOURCE_MIME_TYPE},async()=>({contents:[{uri,mimeType:RESOURCE_MIME_TYPE,text:await readFile(path.join(here,'dist/card.html'),'utf8'),_meta:{ui:{prefersBorder:true}}}]}));
...
}).listen(port,host,()=>console.error(`Wanxiangli MCP: http://${host}:${port}/mcp`));
