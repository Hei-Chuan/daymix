...
const sample=JSON.parse(execFileSync(python,['scripts/daymix.py','--date','2026-09-23','--user-id','demo','--format','json'],{cwd:fileURLToPath(new URL('../',import.meta.url)),encoding:'utf8',env:{...process.env,PYTHONIOENCODING:'utf-8'}}));
...
assert.equal(doc.querySelector('h1').textContent,`今天抽到 · ${sample.rating.rating}`);
assert.ok(!doc.body.textContent.includes('万象历'));
