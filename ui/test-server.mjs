...
try{
 ...
 assert.equal(tool._meta.ui.resourceUri,'ui://wanxiangli/card-v2.1.html');
 const result=await call(2,'tools/call',{name:'get_daily_card',arguments:{date:'2026-09-23',timezone:'Asia/Shanghai',user_id:'demo'}});
 assert.equal(result.structuredContent.schema,'wanxiangli/2');assert.match(result.content[0].text,/今日总结/);
