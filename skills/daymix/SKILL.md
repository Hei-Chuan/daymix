...
如果宿主**已经连接**本项目的 MCP 服务，调用一次 `get_daily_card` 获取完整 `structuredContent`、文本卡和出处。支持 MCP Apps 的宿主还可从同一结果本地展开、收起；普通 MCP 宿主直接使用文本和结构化结果。只有 Skill 时使用脚本文本卡。用户说“展开现实复核”等栏目时，优先复用当前 JSON，用 `daymix.engine.detail`；如果当前 JSON 不在会话里，以同一日期、时区、代号、版本及原有备考/天气输入运行 `--expand yijing|tarot|daoism|astrology|buddhism|christianity|stoicism|reality|all`。不可暗中重抽。
